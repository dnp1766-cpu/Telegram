...
В карточке рецепта есть кнопка **В избранное**. После приготовления, если TheMealDB отдал ссылку, бот показывает **видеорецепт**. Затем предлагает оценку **от 1 до 5 звёзд** кнопками. Оценка сохраняется у избранного блюда; в «Мои рецепты» список идёт по убыванию рейтинга, 5⭐ сверху. Если оценка ещё не стоит, блюдо оказывается внизу.
